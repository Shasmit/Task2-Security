...
export const handleLikeUnlike = async (movieId, reviewID, isLiked, setReviews) => {
  try {
    const url = isLiked
    ...
    const updatedReviewsResponse = await axios.get(`https://localhost:3001/movies/${movieId}/reviews`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
